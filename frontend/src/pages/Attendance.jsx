...
const Attendance = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingId) {
                await axiosInstance.patch(`attendance/${editingId}/`, formData);
            } else {
                await axiosInstance.post('attendance/', formData);
            }
            setIsModalOpen(false);
            fetchData();
        } catch (error) {
            alert("Failed to save attendance. A record might already exist for this student on this date.");
        }
    };
    ...
    const confirmDelete = async () => {
        ...
        try {
            await axiosInstance.delete(`attendance/${deleteConfirmId}/`);
    ...
    return (
        <div className="space-y-6">
            ...
            <DataTable 
                ...
                loading={loading} 
                exportName="Attendance_Log"
            />

            {/* Add/Edit Modal */}
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? "Edit Attendance" : "Mark Attendance"}>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="pt-4 flex justify-end gap-3 border-t border-gray-100">
                        ...
                        <button type="submit" className="px-4 py-2 text-sm font-medium text-white bg-navy-600 rounded-lg hover:bg-navy-700">Save Record</button>
                    </div>
                </form>
            </Modal>
            ...
            <Modal isOpen={!!deleteConfirmId} onClose={() => setDeleteConfirmId(null)} title="Confirm Deletion">
                <div className="flex flex-col items-center justify-center p-4 text-center">
                    ...
                    <p className="text-gray-500 text-sm mb-6">
                        This action cannot be undone. It will permanently remove this attendance record.
                    </p>
